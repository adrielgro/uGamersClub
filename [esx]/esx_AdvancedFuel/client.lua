	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if(hud_form == 1) then
			...
			if(showText) then
				...
				DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.4, "~y~Gas:~s~ " .. round(percent,1).."%", 255, 255, 255, 255, 4, 1)
			end
		else
			if(showBar) then
			...
			if(showText) then
				...
				DrawAdvancedText(text_x, text_y, 0.5, 0.5, 0.5, "~y~Gas:~s~ " .. round(percent,1).."%", 255, 255, 255, 255, 4, 1)
			end
		end
	end
end
